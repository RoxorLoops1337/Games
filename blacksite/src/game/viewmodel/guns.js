...
function tubeOptic(add, opts) {
  ...
  add(latheZ([
    ...
    [rIn, z0],
  ], 22), hex, { round: true, wear: 0.35, ao: 0.45 });
...
function forge(mats) {
  ...
  const self = {
    group, nodes,
    steel: (g, c, o) => bin.add('steel', g, c === undefined ? 0x63686d : c, Object.assign({ wear: 0.55, ao: 0.42 }, o)),
    phos: (g, c, o) => bin.add('phos', g, c === undefined ? 0x4b4f52 : c, Object.assign({ wear: 0.4, ao: 0.42 }, o)),
    poly: (g, c, o) => bin.add('poly', g, c === undefined ? 0x39392f : c, Object.assign({ wear: 0.22, ao: 0.5, grain: 0.05 }, o)),
    rubber: (g, c, o) => bin.add('rubber', g, c === undefined ? 0x23252a : c, Object.assign({ wear: 0.12, ao: 0.55, grain: 0.07 }, o)),
    mesh(geo, mat) {
...
function buildRifle(mats) {
  const f = forge(mats);
  const TAN = 0x6a5f48, BLK = 0x2e3033;
  ...
  f.steel(sweptBlock([0, -0.036, -0.032], [0, -0.062, -0.038], 0.038, 0.030, 0.005,
    [[1.06, 1.10], [1.0, 1.0], [0.97, 0.98]], [0, 0, 1]), 0x5a5f63, { wear: 0.7 });

  // Rail, unbroken from the charging handle out over the handguard.
  railRibs(f.steel, 0.055, -0.300, 0.0350, 0.0212, 0x54585c);
  ...
  const hz0 = -0.155, hz1 = -0.398, hy = 0.002;
  const holes = [];
  for (let i = 0; i < 3; i++) {
    const cz = hz0 - 0.040 - i * 0.070;
    holes.push(roundRectPts(0.048, 0.019, 0.008, 3).map(([a, b]) => [a + cz, b + hy]));
  }
  for (const s of [-1, 1]) {
    const p = sideProfile(
      roundRectPts(hz0 - hz1, 0.055, 0.007, 3).map(([a, b]) => [a + (hz0 + hz1) / 2, b + hy]),
      0.0055, { holes, bevel: 0.0012 });
    p.translate(s * 0.0192, 0, 0);
    f.poly(p, TAN);
  }
  const deck = slab(0.042, 0.008, hz0 - hz1, 0.004, 0.0012);
  deck.translate(0, hy + 0.0255, (hz0 + hz1) / 2);
  f.poly(deck, TAN);
  const floor = slab(0.038, 0.007, hz0 - hz1 - 0.02, 0.004, 0.0012);
  floor.translate(0, hy - 0.0255, (hz0 + hz1) / 2 - 0.008);
  f.poly(floor, TAN);
  railRibs(f.poly, hz0 - 0.004, hz1 + 0.004, hy + 0.0285, 0.0212, 0x3c3a31);
  ...
  ch.steel(latch, 0x6b7074, { wear: 0.9 });
  ...
  mg.poly(sweep(roundRectPts(0.0295, 0.0190, 0.0055, 3), mf), 0x2c2e26, { wear: 0.3 });
  const plate = slab(0.034, 0.010, 0.024, 0.003, 0.0010);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    h.rotateZ(-Math.PI / 2);
    h.translate(0.0150, -0.070 - t * 0.100, -0.043 - t * 0.022);
  ...
  f.poly(beaver, TAN);
  f.steel(latheZ([[0, 0.062], [0.0175, 0.062], [0.0175, 0.185], [0, 0.185]], 16), 0x4a4e52, { round: true, wear: 0.45 });
  for (let i = 0; i < 6; i++) {
  ...
  f.rubber(pad, 0x1e2023);

  // Sights: the optic is the primary, and the folded-down irons stay on the gun
  // because that is what a real rifle looks like.
  const opt = tubeOptic(f.steel, { z0: -0.052, z1: -0.148, y: 0.0640, r: 0.0195, hex: 0x494e52 });
  frontSight(f.phos, -0.396, 0.0285, 0.0130, 0x4d5155);
  ...
  return {
    ...
    hands: {
      ...
      right: { pos: [0, -0.058, 0.036], axis: [0, 0.93, -0.36], palm: [1, 0, 0.30], gripR: 0.019,
               index: [[0.014, 0.036, -0.020], [0.002, 0.041, -0.028]], armDir: [0.42, 0.98, 0.10] },
      // Support hand under the handguard, wrist trailing back toward the body.
      left: { pos: [0, -0.010, -0.276], axis: [0, 0, 1], palm: [-0.48, -0.88, 0], gripR: 0.024, wrap: 2.35,
              armDir: [0.40, 1.0, 0] },
    },
  };
}
...
function buildSMG(mats) {
  ...
  f.steel(spine, 0x4a4f55);
  railRibs(f.steel, 0.046, -0.240, 0.0265, 0.0212, 0x484c52);
  ...
  return {
    ...
    hands: {
      right: { pos: [0, -0.054, 0.032], axis: [0, 0.94, -0.33], palm: [1, 0, 0.28], gripR: 0.018,
               index: [[0.013, 0.034, -0.018], [0.002, 0.039, -0.026]], armDir: [0.42, 0.98, 0.10] },
      ...
      left: { pos: [0, -0.070, -0.192], axis: [0, 1, -0.10], palm: [-0.30, 0, 0.95], gripR: 0.017, wrap: 2.4,
              armDir: [0.30, 0.62, 0.72] },
    },
  };
}
...
function buildDMR(mats) {
  ...
  ], 0.040, { bevel: 0.0018 }), 0x50555a);
  railRibs(f.steel, 0.070, -0.330, 0.0410, 0.0212, 0x4c5055);
  ...
  for (const s of [-1, 1]) {
    ...
    const p = sideProfile(
      roundRectPts(hz0 - hz1, 0.052, 0.008, 3).map(([a, b]) => [a + (hz0 + hz1) / 2, b]),
      0.0055, { holes, bevel: 0.0012 });
    p.translate(s * 0.0205, 0, 0);
    f.poly(p, GRN);
  }
  const ddeck = slab(0.044, 0.008, hz0 - hz1, 0.004, 0.0012);
  ddeck.translate(0, 0.0240, (hz0 + hz1) / 2);
  f.poly(ddeck, GRN);
  railRibs(f.poly, hz0 - 0.004, hz1 + 0.006, 0.0280, 0.0212, 0x2f342b);
  const dfloor = slab(0.040, 0.008, hz0 - hz1 - 0.02, 0.004, 0.0012);
  dfloor.translate(0, -0.0240, (hz0 + hz1) / 2 - 0.008);
  f.poly(dfloor, GRN);
  for (const s of [-1, 1]) {
  ...
  const mf = frameChain(mp, [0, 0, 1]);
  mg.poly(sweep(roundRectPts(0.0290, 0.0230, 0.0055, 3), mf), 0x2a2c26, { wear: 0.3 });
  const dplate = slab(0.034, 0.012, 0.028, 0.003, 0.0010);
  ...
  return {
    ...
    hands: {
      right: { pos: [0, -0.064, 0.042], axis: [0, 0.93, -0.36], palm: [1, 0, 0.30], gripR: 0.020,
               index: [[0.015, 0.038, -0.022], [0.002, 0.043, -0.030]], armDir: [0.42, 0.98, 0.10] },
      left: { pos: [0, -0.014, -0.330], axis: [0, 0, 1], palm: [-0.48, -0.88, 0], gripR: 0.026, wrap: 2.3,
              armDir: [0.40, 1.0, 0] },
    },
  };
}
...
function buildShotgun(mats) {
  ...
  const ringY = rearSight(f.steel, 0.010, 0.0430, 0x3f4347);
  ...
  return {
    ...
    hands: {
      right: { pos: [0, -0.062, 0.036], axis: [0, 0.92, -0.39], palm: [1, 0, 0.32], gripR: 0.020,
               index: [[0.015, 0.036, -0.022], [0.002, 0.041, -0.030]], armDir: [0.42, 0.98, 0.10] },
      // On the pump, so it travels with it when the action cycles.
      left: { pos: [0, -0.052, -0.256], axis: [0, 0, 1], palm: [-0.48, -0.88, 0], gripR: 0.024, wrap: 2.35,
              armDir: [0.40, 1.0, 0] },
    },
  };
}
  ...
  return {
    ...
    hands: {
      right: { pos: [0, -0.048, 0.028], axis: [0, 0.93, -0.37], palm: [1, 0, 0.30], gripR: 0.017,
               index: [[0.012, 0.030, -0.016], [0.001, 0.035, -0.024]], armDir: [0.42, 0.98, 0.10] },
      ...
      left: { pos: [-0.006, -0.068, 0.038], axis: [0, 0.93, -0.37], palm: [-1, 0, 0.24], gripR: 0.028,
              wrap: 1.60, mirror: true, armDir: [0.46, 0.90, 0.10] },
    },
  };
}
