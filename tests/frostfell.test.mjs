...
section('three cards that rewrite a rule, and give it back when they fall');
{
  ...
  {
    clear();
    const front = place(FFa, 'p', 'snowpup', 0, 0, {});
    const back = place(FFa, 'p', 'snowpup', 1, 2, {});
    const efront = place(FFa, 'e', 'snapfrost', 0, 0, {});
    eq(FFa.tickRate(front, GA), 2, 'the front column burns two');
    eq(FFa.tickRate(back, GA), 1, 'the back burns one');
    const bd = place(FFa, 'p', 'backdrift', 1, 1, {});
    eq(FFa.tickRate(front, GA), 1, 'Backdrift standing, the front burns one');
    eq(FFa.tickRate(back, GA), 2, 'and the back burns two');
    eq(FFa.tickRate(efront, GA), 2, 'and the FOES keep their own clock — the aura reads each unit\'s own side');
    bd.alive = false;
    eq(FFa.tickRate(front, GA), 2, 'it falls and the six slots are priced the way they were');
    front.alive = false; back.alive = false; efront.alive = false;
  }

  // COLDBEARER — the room rule, priced rather than deleted
  {
    clear();
    const fill = [];
    for (const s of FFa.freeSlots(GA, 'p').slice()) {
      fill.push(place(FFa, 'p', 'snowpup', s.lane, s.col, {}));
    }
    eq(FFa.freeSlots(GA, 'p').length, 0, 'the line is packed');
    eq(FFa.hasRoom(GA, 'p'), false, 'so it is not warmed');
    eq(FFa.isPacked(GA, 'p'), true, 'and the cold gets in');
    // swap one of the bodies for the Coldbearer, still packed
    const swap = fill[0];
    swap.alive = false;
    const cb = place(FFa, 'p', 'coldbearer', swap.lane, swap.col, {});
    eq(FFa.freeSlots(GA, 'p').length, 0, 'still packed — it takes a slot like anything else');
    eq(FFa.hasRoom(GA, 'p'), true, 'and now the packed line warms anyway');
    eq(FFa.isPacked(GA, 'p'), false, 'the cold no longer picks a victim at random');
    cb.alive = false;
    eq(FFa.hasRoom(GA, 'p'), false, 'and the rule comes straight back when it falls');
  }

  // GRUDGEHORN — the only run-scoped memory in the game
  {
    clear();
    const gh = place(FFa, 'p', 'grudgehorn', 1, 2, {});
    eq(gh.card.kept || 0, 0, 'it starts with nothing banked');
    FFa.addStatus(GA, gh, 'spice', 3);
    FFa.CARDS.grudgehorn.hooks.trigger(GA, gh);
    eq(gh.card.kept, 3, 'what it holds when it swings, it keeps');
    FFa.addStatus(GA, gh, 'spice', 1);
    FFa.CARDS.grudgehorn.hooks.trigger(GA, gh);
    ok(gh.card.kept >= 3, 'the bank never goes down on a smaller swing');
    // a fresh body off the same DECK CARD carries the bank into the next fight
    const again = FFa.mkUnit(gh.card, 'p', 0, 2);
    GA.battle.units.push(again);
    FFa.CARDS.grudgehorn.hooks.deploy(GA, again);
    eq(again.st.spice, gh.card.kept, 'and it walks into the next fight still holding it');
    FFa.CARDS.grudgehorn.hooks.death(GA, again);
    eq(again.card.kept, 0, 'falling empties the bank, which is what it costs');
     ...
     an aura is read off a living body, so no aura may survive its carrier. */
  {
    ...
    for (const id of ['coldbearer', 'backdrift']) {
      ...
      const key = Object.keys(d.aura)[0];
      ok(FFa.auraOn(GA, 'p', key), id + '\'s rule is live while it stands');
      u.alive = false;
      eq(FFa.auraOn(GA, 'p', key), null, id + '\'s rule dies with it');
    }
  }
}
