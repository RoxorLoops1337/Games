// Headless loader for Stable Story (Horse Ranch). Evals the game's inline
// <script> in a stubbed DOM and returns the pure logic surface it hangs on
// globalThis.__HR.
...
export function loadHR() {
  ...
  const html = readFileSync(join(here, '..', 'horse_ranch', 'index.html'), 'utf8');
  const code = html.match(/<script>([\s\S]*)<\/script>/)[1];

  const noop = () => {};
  const ctx = new Proxy({}, { get(_t, k) {
    if (k === 'createLinearGradient' || k === 'createRadialGradient') return () => ({ addColorStop: noop });
    if (k === 'canvas') return { width: 300, height: 100 };
    if (k === 'measureText') return () => ({ width: 6 });
    return noop;
  } });
  const mkEl = () => new Proxy({
    style: {}, classList: { add: noop, remove: noop, toggle: noop, contains: () => false },
    addEventListener: noop, appendChild: noop, remove: noop, setAttribute: noop, getAttribute: () => null,
    getContext: () => ctx, getBoundingClientRect: () => ({ left: 0, top: 0, width: 300, height: 100 }),
    querySelector: () => mkEl(), querySelectorAll: () => [], innerHTML: '', textContent: '', value: '',
    width: 300, height: 100, children: [], dataset: {}, className: '',
